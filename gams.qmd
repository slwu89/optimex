...
Acsets (Attributed C-Sets) are a nifty data structure coming from applied category theory, but its not too far off to think of them just as in-memory relational databases. They are provided in the [ACSets.jl](https://github.com/AlgebraicJulia/ACSets.jl) library.
...
The only code from Catlab we need is the line below the schema where we display it graphically. Note that each "set" has turned into an object in the schema, and that the relations are also objects. There are projection maps from the relations into the sets which are involved in each relation. Each relation also has an arrow into `IntAttr` which will store the results of the binomial random draw that is used to "sparsify" the data.
